...
Matrix filling_with_zeros(Matrix matrix)
{
	int i, j, n, N;
	Matrix new_matrix;
	n = matrix.size;
	...
	while (N < n)
		N *= 2;
	new_matrix = create_empty(N);
	for (i = 0; i < N; i++)
		for (j = 0; j < N; j++)
			if ((i < n) && (j < n))
				new_matrix.ptr_matrix[j][i] = matrix.ptr_matrix[j][i];
			else
				new_matrix.ptr_matrix[j][i] = 0;
	new_matrix.size = N;
	return new_matrix;
}


Matrix removing_zeros(Matrix matrix, int n)
{
	int i, j, N;
	Matrix new_matrix;
	N = matrix.size;
...
Matrix multiplication_by_Strassen(Matrix first_matrix, Matrix second_matrix)
{
	...
	n = first_matrix.size;
	first_matrix = filling_with_zeros(first_matrix);
	second_matrix = filling_with_zeros(second_matrix);
	...
	return removing_zeros(unite(set_of_matrixes), n);
